...
void conv_forward(conv_layer_t* l, vol_t** in, vol_t** out, int start, int end) {
  ...
  double* f_w;
  double* v_w;
  double temp_array_vd16[4];
  double temp_array_vd20[5];
  __m256d simd;
              ...
              if (oy >= 0 && oy < 32) {
                for(int fx = 0; fx < 5; fx++) {
                  ...
                  if(ox >=0 && ox < 32) {
                    f_w = f->w;
                    v_w = V->w;
                    int f_index = (15 * fy) + (3 * fx);
                    int v_index = (96 * oy) + (3 * ox);
                    int fd = 0;
                    while (fd <= 2) {
                      a += f_w[f_index + fd] * v_w[v_index + fd];
                      fd++;
                    }
                  }
                }
              ...
              if (oy >= 0 && oy < 16) {
                for(int fx = 0; fx < 5; fx++) {
                  ...
                  if(ox >=0 && ox < 16) {
                    f_w = f->w;
                    v_w = V->w;
                    int f_index = (80 * fy) + (16 * fx);
                    int v_index = (256 * oy) + (16 * ox);
                    int fd = 0;
                    while (fd <= 12) {
                      simd += _mm256_mul_pd(_mm256_loadu_pd((double*) (f_w + f_index) + fd), _mm256_loadu_pd((double*) (v_w + v_index + fd)));
                      fd += 4;
                    }
                    _mm256_store_pd(temp_array_vd16, simd);
                  }
                }
              }
            }
            a = temp_array_vd16[0] + temp_array_vd16[1] + temp_array_vd16[2] + temp_array_vd16[3];
            a += l_biases;
            set_vol(A, ax, ay, d, a);
          }
        }
              ...
              if (oy >= 0 && oy < 8) {
                for(int fx = 0; fx < 5; fx++) {
                  ...
                  if(ox >=0 && ox < 8) {
                    f_w = f->w;
                    v_w = V->w;
                    int f_index = (100 * fy)+(20 * fx);
                    int v_index = (160 * oy)+(20 * ox);
                    int fd = 0;
                    while (fd <= 16) {
                      simd += _mm256_mul_pd(_mm256_loadu_pd((double*) (f_w + f_index + fd)), _mm256_loadu_pd((double*) (v_w + v_index + fd)));
                      fd += 4;
                    }
                    _mm256_store_pd(temp_array_vd20, simd);
                  }
                }
              }
            }
            a = temp_array_vd20[0] + temp_array_vd20[1] + temp_array_vd20[2] + temp_array_vd20[3] + temp_array_vd20[4];
            a += l_biases;
            set_vol(A, ax, ay, d, a);
          }
        }
...
void net_classify_cats(network_t* net, vol_t** input, double* output, int n) {
  // double ret_val = batch[11][0]->w[CAT_LABEL];
  #pragma omp parallel for
  for (int i = 0; i < n; i++) {
